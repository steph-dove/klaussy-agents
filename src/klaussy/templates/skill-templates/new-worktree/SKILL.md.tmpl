...
3. Take the current repository's folder name (you already know the working directory — don't compute it with a shell substitution like `$(basename $PWD)`, which isn't portable to Windows shells), then run `git worktree add ../<repo-folder>-<branch-name> -b <branch-name> {{BASE_BRANCH}}` to create the worktree from the configured base branch (the trailing start-point keeps the new branch from inheriting whatever branch the user is currently checked out on).
