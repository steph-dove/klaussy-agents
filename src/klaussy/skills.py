...
# Appended to the {{HUMANIZE}} expansion when the block lands in a scaffolded
# skill. Kept out of HUMANIZE_BLOCK itself because `klaussy humanize --rules`
# feeds tools (klaussy-desktop) that have no scaffolded skill to point at, and
# because resolving {{REPO}} here keeps it independent of replace() ordering.
_HUMANIZE_SKILL_POINTER = (
    "**The scrubber is not the humanize pass.** `klaussy humanize` deletes a fixed"
...
def humanize_block(repo_namespace: str) -> str:
    """Return the {{HUMANIZE}} expansion for a skill scaffolded into `repo_namespace`.

    The shared prose rules plus the pointer telling the agent that the full
    rewrite lives in the humanize skill, not in the deterministic CLI.
    """
    return HUMANIZE_BLOCK + "\n\n" + _HUMANIZE_SKILL_POINTER.replace("{{REPO}}", repo_namespace)
