...
def klaussy_init(
    ...
    antigravity, cline, aider, opencode, kimi (or "all"). Defaults to all
    agents. Also writes the PR template and .gitignore entries.
    """
...
def klaussy_settings(repo: str = ".", force: bool = False, agents: str = "all") -> str:
    ...
    copilot, antigravity, cline, aider, opencode, kimi (or "all"); defaults to all.
...
def klaussy_skills(
    ...
    `agents` is a comma-separated list from claude, gemini, cursor, codex,
    copilot, antigravity, cline, aider, opencode, kimi (or "all"); defaults to all. See
...
def klaussy_hooks(repo: str = ".", force: bool = False, agents: str = "all") -> str:
    ...
    gemini, cursor, codex, copilot, antigravity, cline, aider, opencode, kimi
    (or "all"); defaults to all. Aider has no hook mechanism and is skipped (its
    auto-lint/auto-test config is written by the settings step instead). Kimi's
    hooks are emitted as a paste-in snippet (it reads hooks from user config only).
    """
