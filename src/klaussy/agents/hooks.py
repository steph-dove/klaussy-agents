...
# Kimi Code CLI's project-level directory (skills, conventions, klaussy guards).
KIMI_HOME = ".kimi-code"
...
def kimi_hooks(repo: Path, *, force: bool) -> None:
    """Kimi Code CLI: guards committed in-repo, wiring emitted as a paste-in snippet.

    Kimi reads `[[hooks]]` only from the user-level `~/.kimi-code/config.toml`
    (its project-local `local.toml` takes `[workspace]` only, and an unknown key
    fails the whole config load), so the guards are installed under
    `.kimi-code/hooks/` and the blocks pointing at them go to
    `.kimi-code/klaussy-hooks.toml` for the user to paste once.

    Because that config is global, each command resolves the repo root at run
    time inside the `klaussy-hook` launcher and no-ops where the guard isn't
    present — Kimi's four-field hook schema has no per-OS override, so a
    `$(git rev-parse ...)` one-liner would break on Windows. Tool names are
    matched as anchored regexes so `Read` doesn't also catch `ReadMediaFile`.
    Web-fetch scanning is left out: Kimi's `PostToolUse` is observation-only, so
    a `FetchURL` guard could see injected content but not block on it.
    """
    label = "Kimi Code CLI"
    fmt, lint, com = _commit_cmds(repo)
    ...
    if fmt or lint or com:
        ...
        entries.append(("PreToolUse", "^Bash$", COMMIT_GUARD))
    _install_script(repo, f"{hooks_dir}/{COMMENT_GUARD}", "comment_guard.py")
    entries.append(("PreToolUse", "^Bash$", COMMENT_GUARD))
    _install_script(repo, f"{hooks_dir}/{DEPENDENCY_GUARD}", "dependency_guard.py")
    entries.append(("PreToolUse", "^Bash$", DEPENDENCY_GUARD))
    _install_script(repo, f"{hooks_dir}/{READ_GUARD}", "read_guard.py")
    entries.append(("PreToolUse", "^Read$", READ_GUARD))
    # Kimi's Stop is blockable and carries no JSON stop field, so the guard uses
    # the documented exit-2 + stderr form (see the `kimi` dialect in the guard).
    _install_self_review_script(repo, f"{hooks_dir}/{SELF_REVIEW_GUARD}", "kimi")
    entries.append(("Stop", "", SELF_REVIEW_GUARD))

    lines = [
        "# Generated by klaussy. Kimi Code CLI hook wiring.",
        "#",
        "# PASTE THESE INTO ~/.kimi-code/config.toml. Kimi reads [[hooks]] only from",
        "# the user config; the project-local .kimi-code/local.toml accepts a",
        "# [workspace] table only, and an unknown key makes the whole config fail to",
        "# load — so klaussy can't commit these as live settings.",
        "#",
        "# Each command finds the repo root itself and no-ops where the guard isn't",
        "# installed, so leaving them enabled globally is safe. The guards themselves",
        f"# live in {hooks_dir}/ and are refreshed by re-running klaussy.",
        "#",
        "# `klaussy-hook` is the launcher pip puts on PATH when klaussy is installed",
        "# (klaussy-hook.exe on Windows) — it names no interpreter and no shell",
        "# syntax, so these entries work as-is on macOS, Linux, and Windows.",
    ]
    for event, matcher, name in entries:
        lines += ["", "[[hooks]]", f'event = "{event}"']
        if matcher:
            lines.append(f'matcher = "{matcher}"')
        lines += [f"command = '{_cmd(name)}'", "timeout = 60"]

    snippet = repo / KIMI_HOME / "klaussy-hooks.toml"
    if snippet.exists() and not force:
        console.print(f"[yellow]⚠ [{label}] {snippet.name} exists; use --force.[/yellow]")
        return
    ...
    _report(
        ...
        self_review=True,
        read_note=(
            f"hooks are NOT active yet — paste {KIMI_HOME}/klaussy-hooks.toml into "
            "~/.kimi-code/config.toml (Kimi ignores project-local hook config). "
            "Web-fetch scanning is omitted (Kimi's PostToolUse can't block) and "
            f"plan guidance rides {KIMI_HOME}/AGENTS.md instead of a hook"
        ),
    )
