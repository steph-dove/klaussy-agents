...
def klausify_skills(
    ...
    """Scaffold the bundled klausify skills into each selected agent's skills dir.

    Writes one SKILL.md folder per entry in SKILL_NAMES, adapted to each agent.
    `agents` is a comma-separated list from claude, gemini, cursor, codex,
    copilot (or "all"); defaults to all. See src/klausify/skills.py for the set.
    """
