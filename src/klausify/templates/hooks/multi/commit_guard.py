...
Installed by klausify into a target agent's hooks directory and wired to that
...
`klausify hooks --force --agents <agent>`) to change them.
"""
...
def _run(cmd: str) -> int:
    ...
    try:
        ...
        print(f"klausify pre-commit: could not run `{cmd}`: {exc}", file=sys.stderr)
        return 0
...
def main() -> int:
    try:
        ...
        for cmd in (FORMAT_CMD, LINT_CMD, COMMENT_CHECK_CMD):
            ...
            if rc != 0:
                print(
                    f"klausify pre-commit: `{cmd}` failed (exit {rc}). "
                    "Commit blocked.",
                    file=sys.stderr,
                )
                return 2
        ...
        print(f"klausify pre-commit guard error (allowing): {exc}", file=sys.stderr)
        return 0
