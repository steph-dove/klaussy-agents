...
`gemini-cli`, `github-copilot`, `opencode`, `kimi-cli` and others — see
...
Most skills also trigger on their own when the work matches — describe a bug and `debug` picks it up. Eight are explicit-only, because you don't want them firing on their own: `commit`, `pr`, `precommit`, `release`, `restack`, `split-pr`, `new-worktree`, and `document`.
