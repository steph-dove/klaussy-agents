...
Write one shell loop that records how many reviews and comments the request has (the read commands are in the **`httpx-address-review`** skill's forge section), re-counts every few minutes, and exits the first time a count changes or when the window closes. Print only the before and after counts.
