...
You're running **Gemini CLI**. Write permissions to `.gemini/settings.json` (plus secret globs in `.geminiignore`), using the tool allow-list Gemini reads from that settings file. Prefer the local, git-ignored file so one developer's convenience list isn't committed onto the team; only write a shared, committed file if the user asks to apply it team-wide. The rule examples below use Claude Code's `Bash(...)` / `Edit(...)` form, translate them to your file's shape.
