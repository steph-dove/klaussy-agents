...
Then run `klaussy split-carve <plan path> --check "<command>"`, with one `--check` per build, lint and test command from CLAUDE.md (one command each; no pipes or `&&`). It branches each layer off the one below, takes that layer's files as they stand at `<tip>`, commits with hooks off, records the chain for **`httpx-restack`**, and runs the Phase 5 checks. If a changed file is in no layer or in two, it refuses and creates nothing.

A file whose hunks belong to different layers can't be carved whole. Carve those layers by hand with `.gemini/antigravity-cli/plugins/klaussy/skills/httpx-split-pr/manual.md`, then verify the stack with `klaussy split-verify --tip <tip> --layers <bottom,...,top> --check "<command>"`. If `klaussy` isn't on PATH, follow `manual.md` for the whole carve and its checks.
...
**Record the chain for `httpx-restack`.** `split-carve` already did. After a hand carve, run `git config branch.<child>.klaussyParent <parent>` for every layer above the bottom. Restack reads exactly that config and otherwise has to re-derive the topology from ancestry.
