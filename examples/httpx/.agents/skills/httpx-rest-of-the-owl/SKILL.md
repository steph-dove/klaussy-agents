...
1. **Permissions.** If routine dev permissions aren't configured for this worktree yet, run **`httpx-grant-permissions`** so editing, tests, git and the forge CLI don't prompt all run.
...
| :-- | :--- | :--- |
| 1 | **`httpx-plan`** (or **`httpx-implement`**'s lighter planning for a small, single-surface task) | An approved `plan.md`. Ask now if the task leaves a real ambiguity; a wrong assumption costs the whole owl. Keep the plan and any breaking-change notes as session notes per **`httpx-session-context`**. |
| 2 | **`httpx-implement`** | The plan's boxes are ticked and the suite is green. No scope creep beyond the task definition. |
| 3 | **`httpx-review`** against `git diff <base>...HEAD`, then **`httpx-self-review`** | Every finding you agree with is fixed, the rest noted with a reason, suite re-run. Sub-agent returns the verdict line and one line per finding (severity, `file:line`, fix). |
| 4 | **`httpx-qa`** | QA is genuinely clean. Let the skill right-size the evidence; don't hand-pick it. Sub-agent returns each check with pass/fail, the artifacts folder, and any asset URLs for the PR body. |
| 5 | **`httpx-pr`** | The request is open against `<base>`, its number and URL reported. Commit on a topic branch (never straight to `<base>`) and push first. Embed the QA evidence from Phase 4 in the body. |
| 6 | **`httpx-review`** again, now that it's a real PR | Findings fixed, committed, pushed. A PR at rest reads differently: integration seams and the change as a whole surface here. |
| 7 | `waiting.md`, then the adapter's CI commands | Every check is green. |
| 8 | `waiting.md`, then **`httpx-address-review`** | Every comment answered and its thread resolved. Pushing fixes re-triggers CI, so go back to 7 if anything goes red. |
