...
You're running **GitHub Copilot**. Write permissions to `~/.copilot/config.json` and CLI `--allow-tool` flags — Copilot's allow-list is user-level config, not a committed repo file, using its tool-permission entries. Prefer the local, git-ignored file so one developer's convenience list isn't committed onto the team; only write a shared, committed file if the user asks to apply it team-wide. The rule examples below use Claude Code's `Bash(...)` / `Edit(...)` form, translate them to your file's shape.
