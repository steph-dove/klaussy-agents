...
**This skill plans. It does not build.** It ends at the approved `plan.md`; the work itself belongs to **`httpx-implement`**, and the whole plan-to-PR loop to **`httpx-rest-of-the-owl`**. Do not start editing code here, however obvious the first step looks.
