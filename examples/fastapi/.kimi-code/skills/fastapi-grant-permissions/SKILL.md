...
You're running **Kimi Code CLI**. Write permissions to `~/.kimi-code/config.toml` — Kimi reads permission rules only from your user config, so klaussy leaves a paste-in snippet at `.kimi-code/klaussy-permissions.toml`, using its `[[permission.rules]]` entries — a `decision` of `allow`/`deny`/`ask` plus a `pattern` like `Bash(git diff *)`. Prefer the local, git-ignored file so one developer's convenience list isn't committed onto the team; only write a shared, committed file if the user asks to apply it team-wide. The rule examples below use Claude Code's `Bash(...)` / `Edit(...)` form, translate them to your file's shape.
