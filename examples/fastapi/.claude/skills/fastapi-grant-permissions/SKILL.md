...
You're running **Claude Code**. Write permissions to `.claude/settings.local.json` (personal, git-ignored) or `.claude/settings.json` (shared with the team), using a `permissions.allow` / `permissions.deny` array of string rules like `Bash(git *)`, `Edit(**)`, and `Read(**)`. Prefer the local, git-ignored file so one developer's convenience list isn't committed onto the team; only write a shared, committed file if the user asks to apply it team-wide. The rule examples below use Claude Code's `Bash(...)` / `Edit(...)` form, translate them to your file's shape.
