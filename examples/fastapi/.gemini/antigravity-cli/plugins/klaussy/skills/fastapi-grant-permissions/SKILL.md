...
You're running **Google Antigravity**. Write permissions to `.agents/settings.json`, using its allow-list (best-effort — Antigravity's committed permission format isn't fully documented). Prefer the local, git-ignored file so one developer's convenience list isn't committed onto the team; only write a shared, committed file if the user asks to apply it team-wide. The rule examples below use Claude Code's `Bash(...)` / `Edit(...)` form, translate them to your file's shape.
