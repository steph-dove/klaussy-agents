...
A file whose hunks belong to different layers can't be carved whole. Carve those layers by hand with `.gemini/antigravity-cli/plugins/klaussy/skills/fastapi-split-pr/manual.md`, then verify the stack with `klaussy split-verify --tip <tip> --layers <bottom,...,top> --check "<command>"`. If `klaussy` isn't on PATH, follow `manual.md` for the whole carve and its checks.
