...
2. **Give each sub-agent a short prompt.** Every lens sub-agent reads its own instructions and fetches the diff itself, so its prompt is just:
   ...
   Read .opencode/skills/fastapi-review/sub-agents.md, then
   .opencode/skills/fastapi-review/lens-<name>.md, and follow them.
...
Validate every finding before synthesis. The rubric lives in `.opencode/skills/fastapi-review/lens-validation.md`; read it once.

- **More than 6 findings:** split them into batches of 4–6, grouped by file so each validator reads a file once. Spawn one validation sub-agent per batch, all in a single message. Its prompt is the base branch, the batch of findings verbatim, and "Read .opencode/skills/fastapi-review/lens-validation.md and follow it. Write no files." Collect the survivors.
