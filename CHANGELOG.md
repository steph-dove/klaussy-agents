...
- **`klaussy uninstall`.** There was no way to undo `klaussy init` short of deleting
  paths by hand, which is worse than it sounds: klaussy *merges* into `.gitignore` and
  each agent's settings rather than owning them, and it appends its secret block to an
  existing ignore file, so a manual `rm` takes your own entries with it. The command
  removes what klaussy owns, edits what it shares, and reports anything it can't prove
  is its own. It previews by default and asks before deleting (`--dry-run` to stop at
  the preview, `--yes` to skip the prompt).

  Ownership is established per file rather than assumed: a `.klaussy-version` marker, the
  `klaussy_` script prefix, the `# klaussy:` ignore header, a `Generated by klaussy` stamp,
  or a config whose every top-level key is one klaussy writes. A `.cursorignore` you wrote
  keeps its entries and loses only klaussy's block; a settings file with your own hooks
  keeps them; a config carrying a key klaussy never writes is left whole.

  Conventions docs are kept by default, since they get hand-edited and klaussy cannot tell
  your prose from its own. `--all` takes them and returns the repo to its pre-klaussy state,
  verified on a real ten-agent scaffold: nothing left but the source files.
...
- **Type annotations & prompt eval refinements.** Added `from __future__ import annotations`
  across core modules to standardize type evaluation. Refined prompt assertions in restack
  and humanize evaluation suites.
